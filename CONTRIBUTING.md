...
- An R package (`R/`, exposing `check_packages()` and friends).
- A standalone POSIX shell script (`modules/validation.sh`,
...
<https://github.com/rgt47/zzrenvcheck/issues> to confirm the problem
has not already been reported. When opening a new issue, include:

- A minimal reproducible example (a `reprex::reprex()` for R, a copy
  of the project structure and command for the shell script).
- The output of `sessionInfo()` (R) or `bash --version` and the OS
  version (shell).
...
3. Make your changes.
   ...
   - For shell changes, update `modules/validation.sh` and ensure
     POSIX compatibility (no bashisms).
...
6. Run the full check locally:
   ...
   devtools::check()
   ```
7. For shell-script changes, run `shellcheck modules/validation.sh`
   and verify on the supported targets.
8. Update `NEWS.md` with a one-line bullet under the unreleased section.

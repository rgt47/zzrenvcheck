...
#'
#' Adds a GitHub-hosted package entry to renv.lock with proper
#' remote metadata (RemoteType, RemoteHost, RemoteUsername,
#' RemoteRepo, RemoteRef). Fetches version from the repository
#' DESCRIPTION file when possible.
#'
#' @param package Character. Package name.
#' @param remote Character. GitHub remote in "owner/repo" or
...
#'
#' @export
add_github_to_renv_lock <- function(package, remote, path = ".") {
  ...
  ref <- "main"
  if (grepl("@", remote, fixed = TRUE)) {
    parts <- strsplit(remote, "@", fixed = TRUE)[[1]]
    remote <- parts[1]
    ref <- parts[2]
  }

  remote_parts <- strsplit(remote, "/", fixed = TRUE)[[1]]
  if (length(remote_parts) != 2) {
    cli::cli_alert_danger("Invalid remote format: {.val {remote}}")
    return(FALSE)
  }
  username <- remote_parts[1]
  repo <- remote_parts[2]
  ...
  tryCatch({
    ...
    if (!httr::http_error(resp)) {
      ...
      if (length(ver_match) > 0) {
        version <- sub("Version:\\s*", "", ver_match)
      }
    }
  ...
  tryCatch({
    ...
    cli::cli_alert_success(
      "Added {.pkg {package}} ({version}) from GitHub {username}/{repo} to renv.lock"
    )
    TRUE

  }, error = function(e) {
    cli::cli_alert_danger(
      "Failed to add {.pkg {package}} to renv.lock: {e$message}"
    )
    FALSE
  })
}

# Makefile for zzvalidate research compendium
# Docker-first workflow for reproducible research

PACKAGE_NAME = zzrenvcheck
R_VERSION = 4.5.1
TEAM_NAME = rgtlab
PROJECT_NAME = 
...
help:
	...
	@echo "    check-renv            - Full validation: strict + auto-fix (recommended, uses validation.sh)"
	@echo "    check-renv-no-fix     - Validation only, no auto-install (uses validation.sh)"
	@echo "    check-renv-no-strict  - Standard mode (skip tests/, vignettes/, uses validation.sh)"
	@echo ""
	@echo "  Validation with zzrenvcheck R package (requires local R):"
	@echo "    check-pkg             - Full validation with zzrenvcheck (strict + auto-fix)"
	@echo "    check-pkg-no-fix      - Report only, no auto-fix"
	@echo "    check-pkg-no-strict   - Standard mode (skip tests/, vignettes/)"
	@echo "    report-pkg            - Generate detailed status report"
	@echo "    clean-pkg             - Remove unused packages from DESCRIPTION"
	@echo ""
	...
	@echo ""
	@echo "  Docker + zzrenvcheck:"
	@echo "    docker-check-pkg      - Run zzrenvcheck::check_packages() in container"
	@echo "    docker-fix-pkg        - Run zzrenvcheck::fix_packages() in container"
	@echo "    docker-report-pkg     - Run zzrenvcheck::report_packages() in container"
	@echo ""
	@echo "  Cleanup:"
...
# zzrenvcheck R package targets (require local R installation)
# These targets use the zzrenvcheck R package instead of validation.sh
# Advantages: Cross-platform (Windows), better R integration, richer output
# Disadvantages: Requires R installation on host

# Full validation with zzrenvcheck (strict mode + auto-fix)
# Equivalent to: check-renv but uses R package instead of shell script
check-pkg:
	@Rscript -e "zzrenvcheck::check_packages(strict = TRUE, auto_fix = TRUE, verbose = TRUE)"

# Validation only, no auto-fix (report issues without modifying files)
# Equivalent to: check-renv-no-fix but uses R package
check-pkg-no-fix:
	@Rscript -e "zzrenvcheck::check_packages(strict = TRUE, auto_fix = FALSE, verbose = TRUE)"

# Standard mode validation (skip tests/, vignettes/, inst/)
# Equivalent to: check-renv-no-strict but uses R package
check-pkg-no-strict:
	@Rscript -e "zzrenvcheck::check_packages(strict = FALSE, auto_fix = TRUE, verbose = TRUE)"

# Generate detailed status report (returns data frame)
# Shows in_code, in_description, in_renv_lock status for each package
report-pkg:
	@Rscript -e "status <- zzrenvcheck::report_packages(strict = TRUE); print(status)"

# Remove unused packages from DESCRIPTION
# Cleans up packages that are declared but not used in code
clean-pkg:
	@Rscript -e "zzrenvcheck::clean_description(strict = TRUE)"

# Convenience wrapper for fix_packages() (auto-fix with strict mode)
fix-pkg:
	@Rscript -e "zzrenvcheck::fix_packages(strict = TRUE)"
...
# Docker + zzrenvcheck R package targets
# Run zzrenvcheck validation inside Docker container (uses container's R installation)

docker-check-pkg:
	docker run --platform linux/amd64 --rm -v $$(pwd):/home/analyst/project $(PACKAGE_NAME) R --quiet -e "zzrenvcheck::check_packages(strict = TRUE, auto_fix = FALSE, verbose = TRUE)"

docker-fix-pkg:
	docker run --platform linux/amd64 --rm -v $$(pwd):/home/analyst/project $(PACKAGE_NAME) R --quiet -e "zzrenvcheck::fix_packages(strict = TRUE)"

docker-report-pkg:
	docker run --platform linux/amd64 --rm -v $$(pwd):/home/analyst/project $(PACKAGE_NAME) R --quiet -e "status <- zzrenvcheck::report_packages(strict = TRUE); print(status)"
...
.PHONY: all document build check install vignettes test deps check-renv check-renv-no-fix check-renv-no-strict check-renv-ci check-pkg check-pkg-no-fix check-pkg-no-strict report-pkg clean-pkg fix-pkg docker-build docker-build-log docker-push-team docker-document docker-build-pkg docker-check docker-test docker-vignettes docker-render docker-rstudio docker-run r docker-check-renv docker-check-renv-fix docker-check-pkg docker-fix-pkg docker-report-pkg clean docker-clean docker-disk-usage docker-prune-cache docker-prune-all help
